...
class BoundedPriorityQueueTest extends FunSuite {
  ...
  import BoundedPriorityQueue.Implicits._
  import Cmp.Implicits.intCmp

  val intBindPQ = BoundedPriorityQueue.create[Int] _
  ...
  test("Use PQ to sort (as a heap --> bound goes to input size)") {
  ...
  test("PQ of size 3") {
    ...
    val BoundValuePQ = intBindPQ(3)

    val resultPq =
      values
        .foldLeft(BoundValuePQ.empty) {
          case (pq, v) =>
            pq |> BoundValuePQ.insert(v)
        }
    ...
    assert(BoundValuePQ.takeMin(pq3) == None)
  }
